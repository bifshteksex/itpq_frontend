<script lang="ts">
  ...
  let resumes = [
    {
      id: 1,
      title: "Резюме на позицию разработчика",
      views: 120,
      applications: 5,
    },
    { id: 2, title: "Резюме на позицию дизайнера", views: 80, applications: 2 },
  ];
  ...
  >
    ...
    <div class="w-full">
      <section class="w-full flex flex-col items-center">
        <h1 class="text-2xl font-semibold mb-4">Мои резюме</h1>

        <div class="p-4 mb-4 border rounded-md dark:bg-gray-700 dark:text-white">
          <h2>Вы можете загрузить резюме в PDF, RTF, TXT, DOC или DOCX формате!</h2>
          <input accept=".pdf,.rtf,.txt,.doc,.docx" id="upload-resume" type="file">
        </div>
          ...
          <ul class="w-full flex gap-5">
            {#each resumes as resume}
              <a href="/profile/resume">
                <li
                  class="p-4 mb-4 border rounded-md dark:bg-gray-700 dark:text-white"
                >
                  <h3 class="font-bold">{resume.title}</h3>
                  <p>Просмотры: {resume.views}</p>
                  <p>Отклики: {resume.applications}</p>
                </li>
              </a>
            {/each}
          </ul>
        ...
        <a
          href="/profile/resume/new"
          class="mb-6 p-2 bg-green-500 text-white rounded-md hover:bg-green-600"
        >
